...
namespace KLTN.Game.Domain
{
    public sealed class AbilityTargetValidator
    {
        public bool HasEnoughValidTargets(
            ...
            IReadOnlyList<AbilityTargetRequirement>
                requirements)
        {
            ...
            foreach (AbilityTargetRequirement requirement
                in requirements)
            {
            ...
            AbilityTargetSelection selection)
        {
                ...
                in requirements)
            {
                    ...
                    in selectedIds)
                {
                    ...
                    foreach (CardInstance validTarget
                        in validTargets)
                    {
                        if (validTarget.InstanceId ==
                            selectedId)
                        {
                            found = true;
                            break;
        ...
        public IReadOnlyList<CardInstance>
            GetValidTargets(
                MatchState state,
                SeatId abilityOwner,
                ulong sourceCardInstanceId,
            ...
            List<CardInstance> result)
        {
            ...
            foreach (CardInstance card in cards)
            {
                ...
                if (card.Zone != CardZone.Graveyard &&
                    card.IsDead)
                {
                    continue;
                }
