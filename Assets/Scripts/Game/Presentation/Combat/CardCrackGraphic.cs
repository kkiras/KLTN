...
namespace KLTN.Game.Presentation
{
    ...
    public sealed class CardCrackGraphic : MaskableGraphic
    {
        ...
        [Range(4, 12)]
        [SerializeField]
        private int primaryBranchCount = 7;

        [Range(2, 5)]
        [SerializeField]
        private int segmentsPerBranch = 4;
        ...
        protected override void OnPopulateMesh(VertexHelper vertexHelper)
        {
            ...
            if (segments.Count == 0 || revealProgress <= 0f)
            {
                return;
            }
            ...
            if (completeCount >= segments.Count)
            {
                return;
            }
            ...
            AddLine(
                vertexHelper,
                segment.Start,
                Vector2.Lerp(segment.Start, segment.End, partialProgress)
            );
        }
        ...
        private void GenerateSegments(int seed)
        {
            ...
            {
                float angle =
                    Mathf.PI * 2f * branch / primaryBranchCount
                    + RandomRange(random, -0.28f, 0.28f);

                Vector2 current = center;

                for (
                    int segmentIndex = 0;
                    segmentIndex < segmentsPerBranch;
                    segmentIndex++
                )
                {
                    ...
                    float length =
                        radius / segmentsPerBranch * RandomRange(random, 0.75f, 1.2f);
        ...
        )
        {
            ...
            Vector2 end =
                origin + new Vector2(Mathf.Cos(angle), Mathf.Sin(angle)) * length;
